...
from utils import send_mails, generate_email_data, validate_user_credentials, load_json
...
def get_parser():
    ...
    parser.add_argument("--info", action="store_true", help="print debug logs")
    parser.add_argument(
        "-se", "--send-email", action="store_true", help="send email to users"
    )
    return parser
...
if __name__ == "__main__":
    ...
    file_url = os.getenv("JSON_FILE_URL", args.file_url)
    data = load_json(file_url or args.file)
